...
def plot_sa(iter, curr_x, curr_obj, new_x, new_y, T):
    ...
    plt.scatter(new_x, new_y, color='red', label='proposed x')
    plt.axvline(x=curr_x, color='blue', linestyle='-', linewidth=2)
    plt.legend(loc='lower left', fontsize=12)

    plt.text(-0.5, 7.5, f'n = {iter}', fontsize=18, color='black')
    plt.text(-0.5, 6.5, f'Temprature = {T:.4f}', fontsize=18, color='black')
...
def simulated_annealinng(objective, cooling_rate, n_iter):
    ...
    for i in range(n_iter):
        ...
        plot_sa(i, curr_x, curr_obj, new_x, new_obj, T)

        # update current solution iterate
        # 2つ目の条件ではnew - currが小さい方がexpは大きくなり、これはcurrの近傍を探しやすことを意味する
...
for i, rate in enumerate(rates):
    ...
    for diff in [0.03, 0.1, 0.3, 1.0, 3.0]:
        ...
        for _ in range(n_iter):
            ...
            metro = np.append(metro, criterion)
...
pictures[0].save('sa.gif', save_all=True, append_images=pictures[1:], optimize=False, duration=500, loop=0)
