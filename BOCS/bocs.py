...
# added by chatGPT
def ensure_positive_semidefinite(matrix, epsilon=1e-10):
    """共分散行列が正定値半正定値であることを保証します。"""
    min_eigenvalue = np.min(np.linalg.eigvals(matrix))
    if min_eigenvalue < 0:
        matrix += np.eye(matrix.shape[0]) * (abs(min_eigenvalue) + epsilon)
    return matrix
...
class SparseBayesianLinearRegression:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        assert y.ndim == 1, "y should be 1 dimension of shape (n_samples, ), but is {}".format(y.ndim)
        ...
        while (needs_sample):
            ...
            if not np.isnan(_coefs).any():  # non NaN values:
                needs_sample = 0
    ...
    def _order_effects(self, X: np.ndarray) -> np.ndarray:
        ...
        for i in range(2, self.order + 1, 1):
            ...
            offdProd = np.array(list(combinations(np.arange(n_vars), i)))  # n_vars C i
    ...
    def _bhs(self, X: np.ndarray, y: np.ndarray, n_samples: np.int64 = 1,
        ...
        assert X.shape[1] == self.n_coef - 1, "The number of combinations is wrong, it should be {}".format(self.n_coef)
        assert y.ndim == 1, "y should be 1 dimension of shape (n_samples, ), but is {}".format(y.ndim)
        ...
        for i in range(n_samples + burnin):
            ...
            # 共分散行列A_invを計算した後
            A_inv = ensure_positive_semidefinite(A_inv)
...
def bocs_sa(objective, n_vars: np.int64, n_init: np.int64 = 10, n_trial: np.int64 = 100, sa_reruns: np.int64 = 5):
    """ Bayesian Optimization of Combinatorial Structures(BOCS) with Simulated Annealing(SA) """
